...
describe MoviesController do
    ...
    describe 'same_dir' do
        it 'should call the model method that finds the same_dir movies' do
            Movie.should_receive(:find_same_dir).with('Test title')
            get :same_dir, {title: 'Test title'}
        end
        it 'should display all the movies with the same director' do
            movies = ['The Incredibles', 'The Incredibles II']
            Movie.stub(:find_same_dir).with('The Incredibles').and_return(movies)
            get :same_dir, {title: 'The Incredibles'}
            expect(assigns(:find_same_dir)).to eq(movies)
        end
        it 'should return to index and display a warning if movie has no director info' do
            Movie.stub(:find_same_dir).with('Movie with no director info').and_return(nil)
            get :same_dir, {title: 'Movie with no director info'}
            expect(assigns(:find_same_dir)).to eq(nil)
